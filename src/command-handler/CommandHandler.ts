...
import { CommandType, DefaultCommands } from '../enums';
import type {
	AnyCommand,
	CommandUsage,
...
class CommandHandler {
	...
	private async readFiles() {
		...
		const files = this._commandsDir ? getAllFiles(this._commandsDir) : [];
		const validations = [
			...this.getValidations(
		...
		for (const fileData of [...defaultCommands, ...files]) {
			...
			const commandObject: AnyCommand = fileData.fileContents;
	...
	public async runCommand(
		...
		// TOFIX: Avoid using any and fix this
		return await callback(usage as any);
	}
